...
const ORGANIZATION_NAME =
    process.env.ORGANIZATION_NAME || process.env.CIRCLE_PROJECT_USERNAME || config.organizationName;
const PROJECT_NAME =
    process.env.PROJECT_NAME || process.env.CIRCLE_PROJECT_REPONAME || config.projectName;
const IS_PULL_REQUEST = process.env.CI_PULL_REQUEST || process.env.CIRCLE_PULL_REQUEST;
const USE_SSH = process.env.USE_SSH;
// github.io indicates organization repos that deploy via master. All others use production.
const DEPLOYMENT_BRANCH = config.deployBranch;
...
if (!ORGANIZATION_NAME) {
    shell.echo(
        "Missing project organization name. Did you forget to define 'organizationName' in config.js? You may also export it via the ORGANIZATION_NAME environment variable.",
    );
    shell.exit(0);
}
...
if (!PROJECT_NAME) {
    shell.echo(
        "Missing project name. Did you forget to define 'projectName' in config.js? You may also export it via the PROJECT_NAME environment variable.",
    );
    shell.exit(0);
}

if (USE_SSH !== 'true' && !GIT_USER) {
    shell.echo("Missing git user. Did you forget to export the 'GIT_USER' environment variable?");
    shell.exit(0);
}
...
if (shell.exec(`npm run build`).code) {
...
shell.cd(__dirname);
shell.rm('-rf', 'build');
shell.mkdir('-p', `${__dirname}/build`);
shell.cp('-r', path.join(__dirname, '../services/dist'), `${__dirname}/build/${PROJECT_NAME}`);
shell.cp(path.join(__dirname, '../services/package.json'), `${__dirname}/build/${PROJECT_NAME}`);
shell.cp('-r', path.join(__dirname, '../app/dist'), `${__dirname}/build/${PROJECT_NAME}/public`);

shell.cp(
    path.join(__dirname, '../devops/.gitignore.prod'),
    `${__dirname}/build/${PROJECT_NAME}/.gitignore`,
);
shell.cp(
    path.join(__dirname, '../services/.env.example'),
    `${__dirname}/build/${PROJECT_NAME}/.env`,
);

// Save the commit hash that triggers publish-production before checking out to deployment branch
...
fs.copy(
    ...
    (error) => {
        ...
        const commitMessage = CUSTOM_COMMIT_MESSAGE || 'Deploy production';
        const commitResults = shell.exec(
            `git commit -m "${commitMessage}" -m "Deploy production version based on ${currentCommit}"`,
        );
        if (shell.exec(`git push origin ${DEPLOYMENT_BRANCH}`).code !== 0) {
            ...
            shell.echo(`Production is updated and available on '${DEPLOYMENT_BRANCH}'`);
            shell.cd(__dirname);
            shell.rm('-rf', 'build');
            shell.exit(0);
        }
